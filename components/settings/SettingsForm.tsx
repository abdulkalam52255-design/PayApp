...
import type { SettingsViewModel } from '@/lib/view-models/settings';
import { ToggleSwitch } from '@/components/shared/ToggleSwitch';

export function SettingsForm({ initialData }: { initialData: SettingsViewModel }) {
  const [companyName, setCompanyName] = useState(initialData.user.company);
  const [defaultRetainage, setDefaultRetainage] = useState('10');
  const [notificationEmail, setNotificationEmail] = useState(initialData.user.email);
  const [timezone, setTimezone] = useState('America/New_York');
  const [saved, setSaved] = useState(false);
  
  const [emailNotif, setEmailNotif] = useState(initialData.preferences.emailNotifications);
      ...
      <div className={cardClass}>
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2.5">
            ...
            <ToggleSwitch enabled={criticalNotif} onChange={setCriticalNotif} />
          </div>
        </div>
      </div>
